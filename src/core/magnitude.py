...
1. ``estimate_local_magnitude_placeholder`` (antiguo enfoque)
    - Mantiene compatibilidad pero NO es físicamente riguroso.

2. ``estimate_local_magnitude_wa`` (mejorado Wood-Anderson aproximado)
    - Pre-procesa la señal (detrend, demean, taper, filtro banda).
    - Intenta simular una respuesta Wood-Anderson (si ObsPy disponible) o aplica integración heurística.
    - Utiliza la formulación estándar ML = log10(A_mm) - log10 A0(R) con A0(R) piecewise (Hutton & Boore 1987).
    - Distancia R estimada aún con ΔP-S de una sola estación (gran incertidumbre).

IMPORTANTE: Sin remover la respuesta instrumental real ni una localización multi-estación, el resultado sigue siendo preliminar.
"""
...
DEFAULT_BAND = (1.0, 20.0)  # Hz para limpieza básica antes de amplitud ML
...
def estimate_local_magnitude_placeholder(*, picks: Sequence[Dict[str, Any]], trace_data: np.ndarray, trace_sampling_rate: float, station: str) -> MagnitudeResult:
    """Versión previa (no rigurosa). Conservada para comparación."""
    ...
    if delta_ps <= 0:
        return MagnitudeResult(None, None, None, None, "Delta P-S inválida", method="placeholder", warnings=["ΔP-S <= 0"]) 
    ...
    if end_idx <= start_idx:
        return MagnitudeResult(None, None, delta_ps, distance_km, "Ventana inválida", method="placeholder", warnings=["Ventana vacía"]) 
    ...
    try:
        # Usamos la misma fórmula antigua solo para mantener comparabilidad: reformulada como log10A - log10A0*
        ...
        return MagnitudeResult(None, amplitude_mm, delta_ps, distance_km, f"Error ML: {exc}", method="placeholder", warnings=["Excepción cómputo ML"]) 
...
def _bandpass(data: np.ndarray, sr: float, freqmin: float, freqmax: float) -> np.ndarray:
    ...
    # Aplicar transición suave 10% banda
    bw = freqmax - freqmin
    if bw > 0:
        edge = bw * 0.1
...
def estimate_local_magnitude_wa(*, picks: Sequence[Dict[str, Any]], trace_data: np.ndarray, trace_sampling_rate: float, station: str) -> MagnitudeResult:
    """Estimación ML aproximada estilo Wood-Anderson (preliminar).

    Pasos:
      1. Verifica picks P y S.
      2. Distancia R ~ ΔP-S * factor (aún muy crudo).
      3. Preprocesa señal (detrend, taper, banda 1–20 Hz).
      4. Simula Wood-Anderson si es posible; si no, integra según heurística para aproximar desplazamiento y aplica filtro.
      5. Obtiene pico mm en ventana amplia (P a P + min(ΔPS*2, 15 s)).
    ...
    delta_ps = float(s_pick["time_rel"]) - float(p_pick["time_rel"])
    if delta_ps <= 0:
        return MagnitudeResult(None, None, None, None, "ΔP-S inválida", method="wood_anderson", warnings=["ΔP-S <= 0"]) 
    distance_km = _estimate_distance_from_ps(delta_ps)
    sr = float(trace_sampling_rate)
    if sr <= 0:
        return MagnitudeResult(None, None, delta_ps, distance_km, "SR inválido", method="wood_anderson", warnings=["Sampling rate <=0"]) 
    ...
    # Intento de simulación Wood-Anderson
    # Simplificación: si tenemos simulate_seismometer, usamos paz WA sobre desplazamiento aproximado.
    if simulate_seismometer and corn_freq_2_paz:
        try:
            # Heurística: si asumimos datos originales en aceleración (cm/s^2), convertimos a m/s^2, integramos dos veces.
            ...
            # simulate_seismometer espera un Stream; evitamos dependencia creando wrapper mínimo
            # Dado que no tenemos Trace aquí sin modificar interfaz, simplemente aplicamos otro filtrado aproxima.
            # Para mantener compatibilidad, aplicamos un segundo bandpass estrecho alrededor 1 Hz como aproximación.
            disp_wa = _bandpass(disp, sr, 0.5, 8.0)
            wa_mm = disp_wa * 1000.0  # m -> mm
        except Exception:
            warnings.append("Fallo simulación WA -> fallback integración simple")
            wa_mm = data_bp * 0.0  # se corregirá más abajo
    else:
        warnings.append("simulate_seismometer no disponible -> integración heurística")
        # Asumimos aceleración cm/s^2 -> m/s^2
        acc = data_bp / 100.0
        vel = _integrate(acc, sr)
        disp = _integrate(vel, sr)
        wa_mm = disp * 1000.0
    ...
    if wa_mm.size == 0:
        return MagnitudeResult(None, None, delta_ps, distance_km, "Sin datos WA", method="wood_anderson", warnings=warnings + ["Vector vacío"])

    # Ventana de amplitud: desde P hasta P + min(ΔPS*2, 15s)
    ...
    if end_idx <= start_idx:
        return MagnitudeResult(None, None, delta_ps, distance_km, "Ventana amplitud vacía", method="wood_anderson", warnings=warnings + ["Indices invertidos"]) 
    ...
    try:
        ...
        return MagnitudeResult(None, peak_mm, delta_ps, distance_km, f"Error ML: {exc}", method="wood_anderson", warnings=warnings + ["Excepción cómputo ML"]) 

    warnings.append("Distancia basada en ΔP-S una estación (incertidumbre alta)")
